...
General structure:
* `BEELINE/` directory should contain a clone of the forked BEELINE repository, [szysztof17/Beeline](https://github.com/szysztof17/Beeline).
* `config/` directory contains configuration information, including the central `config.yml` file.
* `data/` directory contains all the source data.
* `env/` directory contains conda environments.
* `figures/` directory contains processed data (`figures/data/`), scripts to plot them (`figures/scripts/`), and final plots (`figures/plots/`).
* `src/` directory contains the code to perform the analyses. It is first divided by programming language, then by workflow.
...
We showcased main analyses as jupyter notebooks. They should be run in the general enviroment (`env/sc_analysis.yml`) or a specific one.
...
-   `step_1_data_preparation.ipynb` / env: `sc_analysis`: This notebook shows RNA data preprocessing,
-   `step2_metacells.ipynb`/ env: `seacells`: This notebook demonstrates metacells generation with SEACells and KMenas,
-   `step3_downsampling.ipynb` / env: `sc_analysis`: This notebook demonstrates downsampling experiments.

We also included notebooks showcasing key steps in the analysis for the PBMC10k data and the CellOracle GRN inference method. Please refer to `src/jupyter/` and files named `pbmc_step_X.ipynb` for details:

-   `pbmc_step1_cicero.ipynb` / env: `cicero`, `R`: Running cicero on accessibility data, part of CellOracle workflow,

-   `pbmc_step1.5_process_peaks.ipynb` / env: `co_env` Process cicero output,

-   `pbmc_step2_scRNA.ipynb` / env: `sc_analysis`: RNA data preprocessing,

-   `pbmc_step3_motif_scan.ipynb` / env: `co_env` Motif scanning, part of CellOracle workflow,

-   `pbmc_step4_celloracle.ipynb` / env: `co_env` Running CellOracle.
...
The pipeline uses these to process the scRNA-seq data and build the final network with respect to BEELINE's requirements: \* **TF List File**: (`config["tf_list_file"]`) \* *Example from pbmc.yaml config:* `/home/kl467102/Beeline-238/inputs/hHep/human-tfs.csv` \* **Network File**: (`config["network_file"]`) \* *Example from pbmc.yaml config:* `/home/kl467102/string_dir/9606_protein_links_gene_names_combined_score_700.csv`
